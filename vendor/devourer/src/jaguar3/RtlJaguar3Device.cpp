...
void RtlJaguar3Device::apply_cca_gates_locked(bool primary_disabled,
  ...
  /* 0x524[11] moves with the pair and only with the pair.
   *
   * Deliberate: the two pure states then write exactly the bytes the
   * all-or-nothing path wrote before this split existed, so SetCcaMode is
   * byte-identical and no default changes. What 0x524[11] means on its own
   * is not documented here and was not measured, so a mixed state leaves it
   * at the enabled value rather than guessing. */
