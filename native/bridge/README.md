...
| `radio.tx_power` | runtime TX power as the index/offset model (`TxPower.h`), never dBm: caps + applied state (`step_measured` says whether the slope is measured or documented). Pass `offset_qdb` (relative, shape-preserving), `index_override` (flat index; -1 clears), and/or `reapply` (re-program at the current channel). Vendor-neutral (`IRadio`); a backend without the knobs reports `supported:false`. |
